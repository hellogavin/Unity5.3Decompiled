namespace UnityEditor.VersionControl
{
    ...
    internal class WindowResolve : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            MergeMethod mergeNone = MergeMethod.MergeNone;
            if (GUILayout.Button("merging", new GUILayoutOption[0]))
            {
                mergeNone = MergeMethod.MergeAll;
            }
            if (mergeNone != MergeMethod.MergeNone)
            {
                Task task = Provider.Merge(this.resolveList.SelectedAssets, mergeNone);
                task.Wait();
                if (task.success)
                {
        ...
        private void RefreshList()
        {
            ...
            {
                ListItem item = this.resolveList.Add(null, asset.prettyPath, asset);
                if (flag)
                {
            ...
            if (this.assetList.Count == 0)
            {
                ...
                this.resolveList.Add(null, change.description, change).Dummy = true;
            }
